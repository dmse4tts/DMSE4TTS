...
parser.add_argument(
    '--test_dir', default='./test_files/testsets_youtube',
)
parser.add_argument(
...
if __name__ == "__main__":
    outdir = os.path.join("out", "test_epoch_{}_{}_{}_dpmsolver_retrain_vocoder".format(
        os.path.basename(args.train_config)[:-5], restore_file, testdir.split('/')[1]))
    ...
    model.eval()
    with torch.no_grad():
        for tgfile in os.listdir(os.path.join(testdir, "textgrid")):
            tgpath = os.path.join(testdir, "textgrid", tgfile)
            audiopath = os.path.join(testdir, "wav", tgfile.replace(".TextGrid", ".wav"))
            if not os.path.exists(audiopath): continue
            speaker = "na"
            index = os.path.basename(audiopath)[:-4]
            ret = mypreprocessor.process_utterance(speaker, tgpath, audiopath)
            if not ret: continue
            uttinfo, _ = ret

            _, phonemes_duration, phonemes_code, _, _ = uttinfo.split("|")
            ...
            if use_text:
                ...
                for ph, dur in zip(phonemes_code, phonemes_duration):
                    text_mel.append(np.repeat(
                        np.expand_dims(mydataset.avgmel_dict[ph], 1), dur, 1))
                text_mel = np.concatenate(text_mel, axis=1)

                mel_len = min(
                    [text_mel.shape[1], noisy_mel.shape[1]])

            y = torch.FloatTensor(noisy_mel[:, :mel_len]).unsqueeze(0).cuda()
            ...
            mu = None
            if use_text:
                mu = torch.FloatTensor(text_mel).unsqueeze(0).cuda()
                # save_plot(mu.squeeze().cpu(),
                #           '{}/mu_{}.png'.format(outdir, index))
                mu = mel_normalize(mu)
